...
| 3 | Search Result ↔ analyst Article — ingest path | **Two paths into Articles** | **(A)** Operator Discovery Run → scheduled/triggered ingest → Article in analyst News feed. **(B)** Analyst Search Cluster → Search Query → Query Run → Article directly. Path (A) may copy from operator results; path (B) is independent. |
...
|---------------|----------------|----------------|----------|
| `web_discovery` | Discovery Run (operator) or Query Run (analyst) | Discovery Cluster / Query or Search Cluster / Query | Search Results in `engine_outputs` (operator) or Articles ingest (analyst) |
...
|-------------------------|----------------|-------|
| `web_discovery` (reuse) | Analyst Query Run | `engines.search_cluster_id` / `search_query_id` discriminate from operator runs — see [P1-04 §11.3](./P1-04-admin.md#113-query-run-analyst) |
