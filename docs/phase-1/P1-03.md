...
| Organization member | P1-02 both | §4.2 |
| Organization invite / Integration key | P1-02-Admin | §4.1 |
...
flowchart TD
    ...
    subgraph OrgAdmin["Organization admin · live"]
        ORG[Organization]
        MEM[Org member]
        OC[org ↔ cluster assign]
        OCO[org ↔ company assign]
    end
...
| Do we need Organizations now? | **Yes — admin layer** | `organizations` + 7 related tables shipped in `0012`. Pipeline rows (`clusters`, `companies`) remain global; **scoping via assignment tables**. Read-path enforcement on research/discovery APIs **deferred**. |
...
flowchart TD
    ORG[Organization]
    ORG --> MEM[Organization members]
    ORG --> INV[Organization invites]
    ORG --> KEY[Integration keys]
    ORG --> OC[organization_clusters]
    ORG --> OCO[organization_companies]
    OC --> CL[web_discovery_clusters]
    OCO --> CO[companies]
```

| Parent → Child | Ownership | Without parent? | Cardinality | Required | Backend |
| ---------------- | ----------- | ----------------- | ------------- | ---------- | --------- |
| Organization → Member | Org owns analyst users | No | 1:N | — | `organization_members` |
| Organization → Invite | Org owns pending invites | No | 1:N | — | `organization_invites` |
| Organization → Integration key | Org owns API keys | No | 1:N (one active) | Yes for analyst app | `organization_api_keys` |
| Organization ↔ Cluster | Scoped access | Cluster is global row | **M:N** | No — assign on Access tab | `organization_clusters` |
| Organization ↔ Company | Scoped access | Company is global row | **M:N link; company exclusive** | No — assign on Access tab | `organization_companies` — `UNIQUE(company_id)` |

**Scoping rules (locked):**

- One org → many clusters and many companies.
- One cluster → many orgs (shared cluster assignment allowed).
- **One company → at most one org** (exclusive).

**Not yet enforced:** `GET /api/research/*` and Web Discovery routes do not filter by org. Assignment is stored; analyst integration key validation on read APIs ships later.

GM operators use `X-Admin-Token` — not stored as org members.

### 4.2 Organization member

| Parent → Child | Ownership | Without parent? | Cardinality | Required | Backend |
| ---------------- | ----------- | ----------------- | ------------- | ---------- | --------- |
| Member → Run (triggered) | Member may initiate runs (future) | Yes — runs have no user FK today | 1:N | Optional | — |
| Invite → Member | Invite converts on accept | Invite row kept | 1:1 | — | `organization_invites` → `organization_members` |

Runs do not record `triggered_by_user_id` yet.
