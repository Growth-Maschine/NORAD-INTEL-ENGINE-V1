...
| 5 | Research Signal ↔ Company Signal | **Same `signals` row** | Extracted during deep research, displayed on company detail. Distinct from legacy **News Signal** (Phase 2 feed object). |
...
| 7 | Company — merge on complete | **Yes — one canonical `companies` row per domain** | Free-text at run start resolves on complete. `runs.company_id` and `runs.card_id` filled at end. Domain dedupes on `companies.domain`. |
| 8 | Results API hydration | **Yes — canonical enrich from `articles`** | `GET .../queries/:id/results` calls `_hydrate_results_from_articles` so UI always shows `summary`, `mentioned_companies`, cleaned `body_text` from Article when available. |
...
| News Signal vs Research Signal? | **Different parents (Phase 2 vs built)** | News Signal would belong to a feed Article. Research / Company Signal is a `signals` row on a Company Card. |
...
| Article → Deep research action | User action on `mentioned_companies` | N/A | 1:N companies per article | Optional |
...
* **Ownership:** Article owns `summary`, `mentioned_companies`, `body_text`.
...
flowchart TD
    ...
    RS[Research Signal / Company Signal]
    SO[Source]
    RE[Run Event]
    EC[Engine Call]
...
|--------------|------------|
| Article ↔ Company (mentions) | **Resolved:** `articles.mentioned_companies[]` JSON array. Company does not own articles. M:N via array. |
...
|----------|---------------|------------------|
| Web Discovery — enrich | Article | `articles.summary`, `articles.mentioned_companies` |
| Deep Research — synthesise | Company Card | `cards.card` JSONB, `signals` rows |
...
|---------------|----------------|----------------|----------|
| `web_discovery` | Run | Cluster / Query | Search Results in `engine_outputs`; new `articles` rows |
| `research` | Deep Research Run | Deep research action, API | Company, Card, Signals, Sources |
